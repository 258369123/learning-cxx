...
int main(int argc, char **argv) {
    ...
    ASSERT(d.virtual_name() == 'C', MSG);    //使用final修饰符 所有D无法重写
    ASSERT(a.direct_name() == 'A', MSG);
    ASSERT(b.direct_name() == 'B', MSG);
    ASSERT(c.direct_name() == 'C', MSG);
    ASSERT(d.direct_name() == 'D', MSG);

    A &rab = b;    //虚函数调用基于实际对象类型，因此调用的是 B::virtual_name()
    B &rbc = c;  
    C &rcd = d;
    ...
    ASSERT(rab.direct_name() == 'A', MSG);     //非虚函数调用基于引用的静态类型，因此调用的是 A::direct_name()
    ...
    ASSERT(rbd.direct_name() == 'B', MSG);                //看继承链的函数修饰符 
                                                          //看是否是虚函数
                                                          //虚函数就是引用实际类型    非虚函数就是引用被引用类型
